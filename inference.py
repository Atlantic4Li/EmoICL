...
def parse_args():
    ...
    parser.add_argument('--dataset', default='Intentonomy', type=str, choices=['Intentonomy','EmotionROI','ArtPhoto','EmoSet','StanfordCars','OxfordFlowers17','StanfordDogs','CUB_200_2011','Oxford-IIIT_Pet'])
...
def eval_questions(args, query_meta, support_meta, model, tokenizer, processor, engine, strategy, n_shot):
    ...
    if args.ft:
        with open(f'{args.dataDir}/{args.dataset}/ft_{args.sim_path}', 'rb') as f:
            similarity_data = pickle.load(f)
        with open(f'{args.dataDir}/{args.dataset}/ft_{args.support_path}', 'rb') as f:
            support_features = pickle.load(f)
        with open(f'{args.dataDir}/{args.dataset}/ft_{args.query_path}', 'rb') as f:
        ...
        with open(f'{args.dataDir}/{args.dataset}/{args.query_path}', 'rb') as f:
            query_features = pickle.load(f)
    with open(f'{args.dataDir}/{args.dataset}/{args.text_image_sim}', 'rb') as f:
        text_image_sim = pickle.load(f)
    ...
    for query in tqdm(query_meta):
        ...
        n_shot_support = ICL_utils.select_demonstration(support_meta, n_shot, args.dataset, strategy, query, similarity_data, support_features, query_feature, args.balance_threshold, text_image_sim)
